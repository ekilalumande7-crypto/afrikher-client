        ...
        {hasPosts ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {blogPosts.map((post, index) => (
              <Link
                key={post.id}
                href={`/blog/${post.slug}`}
                className={`group flex h-full flex-col overflow-hidden border border-black/6 bg-white/80 transition-transform duration-500 hover:-translate-y-1 ${index === 1 ? "xl:translate-y-4" : ""}`}
              >
                ...
                <div className="flex flex-1 flex-col px-6 pt-5 pb-6">
                  ...
                  <h3 className="mt-4 line-clamp-3 font-display text-[1.8rem] md:text-[1.95rem] leading-[1.08] tracking-[-0.015em] text-[#0A0A0A] transition-colors duration-300 group-hover:text-[#8A6E2F]">
                    {post.title}
                  </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {[0, 1, 2].map((item) => (
              <div
                key={item}
                className={`overflow-hidden border border-black/6 bg-white/80 ${item === 1 ? "xl:translate-y-4" : ""}`}
              >
                <div className="aspect-[4/3] animate-pulse bg-[#E8E0D1]" />
