...
import { Package, Download, ChevronDown, ChevronUp, BookOpen, Truck, CheckCircle } from "lucide-react";
...
const statusLabels: Record<string, { label: string; color: string; icon: any }> = {
  ...
  delivered: { label: "Livré", color: "bg-green-200 text-green-800", icon: CheckCircle },
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-8 md:p-10 border border-[#2A2A2A]/10 shadow-sm">
        ...
        <p className="text-[#9A9A8A] text-sm mb-8">Retrouvez toutes vos commandes et téléchargez vos magazines.</p>
                        ...
                        {canDownload(order) && order.items?.some(isDigitalItem) && (
                          <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded flex items-center gap-3">
                            ...
                            <p className="text-sm text-green-700">
                              Votre paiement a été confirmé. Vous pouvez télécharger vos magazines en cliquant sur le bouton PDF.
                            </p>
                          </div>
                        )}
                        ...
                        {order.status === "pending" && (
                          <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded flex items-center gap-3">
                            ...
                            <p className="text-sm text-yellow-700">
                              Votre commande est en cours de traitement. Le téléchargement sera disponible après confirmation du paiement.
                            </p>
                          </div>
                        )}
                      </div>
